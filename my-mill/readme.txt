This directory was generated by stepconf 1.1 at Wed Sep 18 17:02:48 2019
and then modified by hand for a 4-axis machine (linear X, Y, and Z axes 
plus a rotary A axis)

NOTES for .ini file

1) SCALE coefficients for X, Y, and Z axes are set up so that -X is
   toward left of work, -Y is toward operator's side of work, 
   and -Z is deeper into the work.

2) SCALE coefficient for A axis are set up so that rotary table 
   angle reads the same in degrees as AXIS digital readout.
